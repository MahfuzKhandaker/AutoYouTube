...
class BuildThumbnail():
    ...
    def create_snapshots_from_videos(self, target_string):
        ...
        compliation_vids_path = './videos/outputs/luxuriousvideos/'
        thumbnails_made_path = './final_thumbnails/'
        if not os.path.exists(thumbnails_made_path):
            os.makedirs(thumbnails_made_path)
    ...
    def joining_thumbnails(self, target_string):
        ...
        for folder in list_of_folders_of_images_compilation:
            ...
            monkeyMind_logo = Image.open('./logo/MONKEYMIND.png')
            monkeyMind_logo.thumbnail((300, 300), Image.ANTIALIAS)
            whole_image = Image.new('RGB', (total_width, total_height))
            whole_image.paste(new_image)
        
            total_x_space = total_width-monkeyMind_logo.size[0]
            total_y_space = total_height-monkeyMind_logo.size[1]

            whole_image.paste(monkeyMind_logo, (total_x_space, total_y_space), mask=monkeyMind_logo)

            # work with center
            x = whole_image.width//2
            y = whole_image.height//2
            # Draw on shape text
            draw = ImageDraw.Draw(whole_image)
            draw.text(xy=(x, y), text="LUXURY LIVING ROOM VIDEO", fill=(224, 224, 224), font=ImageFont.truetype("../impact.ttf", 90), stroke_width=3, stroke_fill=(0, 0, 0), anchor='mm')
